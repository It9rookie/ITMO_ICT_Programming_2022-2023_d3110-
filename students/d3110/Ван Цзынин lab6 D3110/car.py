    ...
    for car_cls in online_all_list:
        ...
        if car_cls.get("id"):
            ...
            if car_cls.get("diff_config_with_no_pic"):
                ...
                configure = ""
            car_type_dict["Название автомобиля"] = car_name
            car_type_dict["Тип транспортного средства"] = car_type
            car_type_dict["Официальная ориентировочная цена"] = price
            car_type_dict["Котировка дилера"] = dealer_price
            car_type_dict["Справочная цена владельца"] = owner_price
            car_type_dict["Тип модернизации автомобиля"] = upgrade
            car_type_dict["Метка транспортного средства"] = tags
            car_type_dict["Конфигурация автомобиля"] = configure
...
def save_json(car_name, text):
    ...
    with open(car_name+".txt", "w", encoding="utf-8") as f:
        f.write(json_text)
        print(car_name+"Успех")

# 启动函数
def main(car_name, city_name):
    car_id = get_car_id(car_name=car_name, city_name=city_name)
    carinfo = {
        "Детали автомобиля": get_car_detail(car_id=car_id, city_name=city_name),
        "Информация о сделке с владельцем": get_car_frind_comment(car_id=car_id)
    }
